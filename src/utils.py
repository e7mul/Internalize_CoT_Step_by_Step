...
def setup_distributed():
    ...
    if world_size > 1:
        ...
        try:
            ...
            print(
                f"[Rank {rank}] Successfully initialized distributed training on GPU {local_rank}"
            )

            # Test basic communication
            if world_size > 1:
                test_tensor = torch.tensor(
                    [rank], dtype=torch.float32, device=f"cuda:{local_rank}"
                )
                ...
                if test_tensor.item() == expected_sum:
                    ...
                    print(
                        f"[Rank {rank}] Communication test failed! Expected {expected_sum}, got {test_tensor.item()}"
                    )
...
def load_experiment_config(rpath: str):
    ...
    class ExperimentConfig:
        def __init__(self, config_data):
            ...
            self.use_temperature_scaling = config_data.get(
